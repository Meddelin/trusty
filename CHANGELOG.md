...
### Added — UX & design overhaul (post-review)
- **Custom routing lists** — the built-in "blocked in Russia" preset became one entry of a general mechanism: add your own lists from a raw URL (GitHub etc.) or a local file, with a validation preview ("Found N valid entries"). URL lists **auto-update at connect time** when older than 24 hours (bounded by an 8-second budget; falls back to the cached copy offline, re-downloads if the cache file vanished). Each list can apply in Selective mode (route via VPN), General mode (bypass VPN) or both.
- **Servers got their own tab** — a card list where one click switches the active server (no more dropdown digging), the pencil opens an inline editor, and shared network settings (DNS) live at the bottom. The Settings tab now holds only application options (log level, close behavior).
...
- **Settings restructured** into two scoped cards: *Server* (per-server connection fields, Advanced expansion, explicit Save that lights up only when there are changes) and *Application* (DNS, log level, close behavior — shared by all servers, applied immediately). DNS and log level are now truly app-global keys with one-time migration. New **Add-server dialog** replaces the placeholder flow (nothing is committed until confirmed), plus a **discard-unsaved-changes guard** when switching servers. Servers can have display names. The nav tab is now called **Deploy**.
...
### Added
- **Multiple servers** — Trusty now keeps a list of servers. Settings has a server dropdown with add/delete buttons, and the Home screen shows a quick switcher above the Connect button when more than one server is saved. Switching replaces only the connection fields (host, credentials, protocol tweaks); app-wide settings — DNS, log level, VPN mode, split-tunneling lists — are preserved. Each server's password lives under its own OS-keystore key. The existing single config is migrated into the list automatically.
...
- **Multiple DNS upstreams** — the DNS field accepts several upstreams separated by commas (e.g. a DoH resolver plus a plain-IP fallback); all of them are written to `dns_upstreams`.
- **Ready-made routing preset** — a one-click "sites blocked in Russia" toggle in Split Tunnel (selective mode), in the spirit of roscomvpn-routing: downloads the maintained [itdoginfo/allow-domains](https://github.com/itdoginfo/allow-domains) lists (blocked domains + Telegram/Discord subnets, ~1200 entries), stores them locally (`client/routing_preset.lst`) and merges them into the exclusions at connect time. A refresh button re-downloads the latest lists; the user's own domain list stays separate.
